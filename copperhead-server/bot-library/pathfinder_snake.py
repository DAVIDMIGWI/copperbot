...
"""
Pathfinder Snake — CopperHead bot using BFS pathfinding to food and flood-fill
survival when no route exists.

Strategy
--------
1. Build a danger map from snake bodies (excluding tail segments, like CopperBot).
2. Prefer the nearest food (Manhattan), with a slight preference for apples.
3. BFS from the head over walkable cells to find the shortest path to that food.
   If a path exists, take the first step.
4. If no path to any food, choose the safe non-reversing move that maximizes
   reachable space (BFS flood count capped for speed).
5. Head-on tile handling mirrors CopperBot: predict opponent's next head cell
   and score that tile using difficulty (avoid vs seek when longer).

Difficulty scales random noise on tie-breaking and head-on aggression, similar
to copperbot.py.
"""
...
def bfs_path(
    ...
    """Return path [start, ..., goal] or None if unreachable."""
    if start == goal:
        return [start]
...
def flood_reachable(
    ...
    """Count cells reachable from start (4-neighbor BFS), capped for performance."""
...
class PathfinderSnake:
    """Autonomous player: BFS to food, flood-fill survival fallback."""
    ...
    def __init__(
        ...
        self.name = name or f"Pathfinder L{difficulty}"
    ...
    def calculate_move(self) -> str | None:
        ...
        foods = self._foods()
        # Sort foods by distance (apples slightly preferred)
        ...
        for food in foods_sorted:
            ...
            if path and len(path) >= 2:
                step = path[1]
                dx, dy = step[0] - head[0], step[1] - head[1]
                for d, (vx, vy) in DIR_VEC.items():
                    if (vx, vy) == (dx, dy) and d != OPPOSITES.get(current_dir):
                        path_first_step = d
                        break
                if path_first_step:
                    break
        ...
        def score_move(m: dict) -> float:
            ...
            s += flood_reachable((m["x"], m["y"]), dangerous, w, h) * 3.0
            for f in foods:
...
async def main() -> None:
    parser = argparse.ArgumentParser(description="Pathfinder Snake — CopperHead bot")
    parser.add_argument(
        "--server",
        "-s",
    ...
    parser.add_argument("--name", "-n", default=None, help="Bot display name")
